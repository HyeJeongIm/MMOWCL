...
class BaseLearner(object):
    ...
    def _choose_optimizer(self):
        ...
        if 'Gyro' not in self._modality and 'Acce' not in self._modality:
            optimizer = torch.optim.SGD(param_groups,
                                        self._lr,
                                        momentum=self._momentum,
                                        weight_decay=self._weight_decay)

        if 'Gyro' in self._modality and 'Acce' in self._modality:
            param_groups1 = [
                {'params': filter(lambda p: p.requires_grad, self._network.backbone.gyro.parameters())},
                {'params': filter(lambda p: p.requires_grad, self._network.backbone.acce.parameters())},
                {'params': filter(lambda p: p.requires_grad, self._network.fusion.parameters())},
                {'params': filter(lambda p: p.requires_grad, self._network.fc.parameters())},
                ]
            if 'RGB' in self._modality:
                optimizer_rms = torch.optim.RMSprop(param_groups1,
                                                    self._lr,
                                                    weight_decay=self._weight_decay)
                
                optimizer_sgd = torch.optim.SGD(param_groups,
                                                self._lr,
                                                momentum=self._momentum,
                                                weight_decay=self._weight_decay)
                        
                optimizer = [optimizer_rms, optimizer_sgd]
            else:
                optimizer = torch.optim.RMSprop(param_groups1,
                                                self._lr,
                                                weight_decay=self._weight_decay)

        else:
            if 'Gyro' in self._modality:
                if len(self._modality) > 1:
                    param_groups1 = [
                {'params': filter(lambda p: p.requires_grad, self._network.backbone.gyro.parameters())},
                {'params': filter(lambda p: p.requires_grad, self._network.fusion.parameters())},
